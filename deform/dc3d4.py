...
def dc3d4(alpha, x, y, dip, al1, al2, aw1, aw2, disl1, disl2):
    """
    Compute displacements (ux, uy, uz) for a buried fault in a semi-infinite medium.
    ...
    # Calculate medium and fault dip constants
    ...
    for k in range(2):
        ...
        for j in range(2):
            ...
            if c0_cd != F0:
                xx = np.sqrt(xi**2 + q**2)
                ai4 = (xi != 0) * ((F1 / c0_cdcd) *
                                  (xi / rd * c0_sdcd + F2 * np.arctan((et * (xx + q * c0_cd) +
                                                                      xx * (c2_r + xx) * c0_sd) /
                                                                     (xi * (c2_r + xx) * c0_cd))))
